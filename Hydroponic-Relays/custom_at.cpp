...
int pump_time_handler(SERIAL_PORT port, char *cmd, stParam *param);
...
{
	return api.system.atMode.add((char *)"PUMP",
								 (char *)"Set/Get the interval the water pump is on and off in seconds",
								 (char *)"PUMP", pump_time_handler,
								 RAK_ATCMD_PERM_WRITE | RAK_ATCMD_PERM_READ);
}

/**
 * @brief Handler for pump on/off time AT commands
 *
 * @param port Serial port used
 * @param cmd char array with the received AT command
 * @param param char array with the received AT command parameters
...
int pump_time_handler(SERIAL_PORT port, char *cmd, stParam *param)
{
	uint32_t new_on_time, new_off_time;
	if (param->argc == 1 && !strcmp(param->argv[0], "?"))
	{
		AT_PRINTF("%s=%ld:%ld", cmd, custom_parameters.pump_on_time / 1000, custom_parameters.pump_off_time / 1000);
	}
	else if (param->argc == 2)
	{
		uint32_t old_on_time = custom_parameters.pump_on_time;
		uint32_t old_off_time = custom_parameters.pump_off_time;

		// Check for valid numbers
		if (0 != at_check_digital_uint32_t(param->argv[0], &new_on_time))
		{
			MYLOG("AT_CMD", "Requested on time wrong");
			return AT_PARAM_ERROR;
		}
		if (0 != at_check_digital_uint32_t(param->argv[1], &new_off_time))
		{
			MYLOG("AT_CMD", "Requested of time wrong");
			return AT_PARAM_ERROR;
		}
		MYLOG("AT_CMD", "Requested on time %ld s", new_on_time);
		MYLOG("AT_CMD", "Requested off time %ld s", new_off_time);

		if (new_on_time < 60)
		{
			MYLOG("AT_CMD", "ON time too short, should be at least 60 seconds");
			return AT_PARAM_ERROR;
		}
		if (new_off_time < 120)
		{
			MYLOG("AT_CMD", "OFF time too short, should be at least 120 seconds");
			return AT_PARAM_ERROR;
		}
		custom_parameters.pump_on_time = new_on_time * 1000;
		custom_parameters.pump_off_time = new_off_time * 1000;
		...
		// Save custom settings if needed
		if ((old_on_time != custom_parameters.pump_on_time) || (old_off_time != custom_parameters.pump_off_time))
		{
			save_at_setting();
		}
	}
	...
	{
		MYLOG("AT_CMD", "Wrong number of arguments");
		return AT_PARAM_ERROR;
	}
...
bool get_at_setting(void)
{
	...
	if (flash_value[0] != 0xAA)
	{
		...
		custom_parameters.pump_on_time = 0;
		custom_parameters.pump_off_time = 0;
		save_at_setting();
		return false;
	}
	custom_parameters.send_interval = temp_params.send_interval;
	custom_parameters.pump_on_time = temp_params.pump_on_time;
	custom_parameters.pump_off_time = temp_params.pump_off_time;

	MYLOG("AT_CMD", "Send interval found %ld", custom_parameters.send_interval);
	MYLOG("AT_CMD", "Pump on time found %ld", custom_parameters.pump_on_time);
	MYLOG("AT_CMD", "Pump off time found %ld", custom_parameters.pump_off_time);
	return true;
}
