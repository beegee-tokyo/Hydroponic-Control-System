...
function lppDecode(bytes) {
	...
	var sensor_types = {
		...
		2: { 'size': 2, 'name': 'analog_in', 'unsigned': true, 'divisor': 100 },
	...
	while (i < bytes.length) {
		...
		switch (s_type) {
			...
			case 134:   // Gyrometer
				...
				if (s_no === 4) {
					s_value['x'] = s_value['x'] * 10;
					s_value['y'] = s_value['y'] * 10;
					s_value['z'] = s_value['z'] * 10;
				}
				break;
...
function Decode(fPort, bytes, variables) {
	...
	//    let pstOffset = +480; // this is the offset for the Philippine Standard Time timezone
	//    let adjustedTime = new Date(date.getTime() + (pstOffset + timezoneOffset) * 60 * 1000);
	//
	//	response['ts'] = adjustedTime.getFullYear() + '/' + (adjustedTime.getMonth() + 1) + '/' + adjustedTime.getDate() + ' ' + adjustedTime.getHours() + ':' + ('0' + (adjustedTime.getMinutes() + 1)).slice(-2) + ':' + ('0' + (adjustedTime.getSeconds() + 1)).slice(-2);
